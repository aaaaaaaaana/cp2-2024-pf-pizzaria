...
public class ProdutoResource implements ResourceDTO<ProdutoRequest, ProdutoResponse> {
    ...
    public ResponseEntity<Collection<ProdutoResponse>> findAll(
        ...
        var encontrado = produtoService.findAll(example);



        var response = encontrado.stream().map(produtoService::toResponse).toList();
    ...
    @Transactional
    @PostMapping
    public ResponseEntity<ProdutoResponse> save(@RequestBody PizzariaRequest produto) {
